...
async function init() {
  ...
  // for each recipe, add it to the <main> element
  ...
  if ('serviceWorker' in navigator) {
    ...
    window.addEventListener('load', async() => {
      ...
      const registration = await navigator.serviceWorker.register('./sw.js')
        ...
        .then(registration => {
          console.log('Service Worker is registered with scope successfully:', registration.scope);
        })

        // B5. TODO - In the event that the service worker registration fails, console
        //            log that it has failed.
        .catch(error => {
          console.error('Service Worker registration has failed in this browser:', error);
        });
    });
  } 
}
...
function addRecipesToDocument(recipes) {
  ...
  recipes.forEach((recipe) => {
    let recipeCard = document.createElement('div'); // use 'div' or any suitable tag
    recipeCard.className = 'recipe-card'; // Ensure this class name matches your CSS
    recipeCard.data = recipe; // This assumes you have some method to process and display data
